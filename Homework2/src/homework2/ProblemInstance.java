...
public class ProblemInstance {
    ...
    public void addLocation(Location location) {
        ...
        if (this.existsLocation(location) == false) {
            ...
            System.out.println("[instance " + this.instanceNumber + "] " + "You cannot add " + location.getName() + "(location) because it already exists in the problem");
        }
    }
    ...
    public void addRoad(Road road) {
        ...
        if (this.existsRoad(road) == false) {
            ...
            System.out.println("[instance " + this.instanceNumber + "] " + "You cannot add the road with the id: " + road.getIdRoad() + " because it already exists in the problem");
        }
    }
    ...
    /**
     * This method checks if the current instance of the problem is valid. The instance is valid if for each road, the road length is greater than the euclidian distance between every 2 location coordinates that the road crosses
     * This  method prints appropriate messages for each case.
     *
     * @return true if the instance is valid, false otherwise.
     */
    public boolean checkValidityOfInstance() {
        boolean result = true;
        for (int i = 0; i < this.countRoads && result == true; i++) {

            double distance = this.computeDistanceBetweenLocations(this.roads[i].endPoints[0], this.roads[i].endPoints[1]);
            if (distance > this.roads[i].getLength()) {
                result = false;
                System.out.println("[instance " + this.instanceNumber + "] " + "A road with length less than the euclidian distance between the 2 end points location coordinates was found. Therefore this instance is not valid");
            }
        }
